...
const ProfilePage = () => {
  ...
  const { user, addProfilePic } = useUserStore();
  ...
  const handleFileChange = async (e) => {
    ...
    if (!validFileTypes.find((type) => type === file.type)) {
      setFileTypeError("File must be JPG, JPEG or PNG");
    ...
    reader.onload = async () => {
      ...
      await addProfilePic({ file: base64Image, type: file.type });
    };
  };
  ...
  return (
    <div
      className={
        "flex container flex-col w-full items-center justify-center h-lvh"
      }
    >
      <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="flex flex-col items-center pb-10">
          <img
            ...
            src={selectedImage || user.profilePic || img}
            alt=""
          />
          <input
            id="imageInput"
          ...
          />
          <button onClick={handleFileButton} className={"closeButton"}>
            <FaCamera size={20} />
          </button>
          {fileTypeError && <p className={"text-red-500"}>{fileTypeError}</p>}
